...
export function generateAutomaticSummaries(entries = [], existing = [], now = new Date(), timeZone = getTimeZone()) {
  const written = entries.filter(isWrittenEntry);
  if (!written.length) return existing.filter((summary) => !summary.autoGenerated);

  const currentWeekStart = weekStartKey(now, timeZone);
  const lastCompletedWeekStart = addDaysToKey(currentWeekStart, -7);
  const byId = new Map(
    existing
      .filter((summary) => !summary.autoGenerated)
      .map((summary) => [summary.id, summary]),
  );

  const summary = generateWeeklySummaryForStart(written, lastCompletedWeekStart);
  if (summary && !byId.has(summary.id)) {
    byId.set(summary.id, { ...summary, autoGenerated: true });
  }
