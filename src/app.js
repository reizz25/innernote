...
function renderCoverSide() {
  ...
  return `
    <section class="side-block">
      ...
      ` : '<p class="muted-copy">回顾生成后，会按日期放在这里。</p>'}
    </section>
  `;
}
...
function renderReviewLinks() {
  ...
  if (!visibleSummaries.length) {
    return '<p class="quiet-copy">回顾生成后，会按日期放在这里。</p>';
  }
...
function reviewSummariesForSidebar() {
  return state.summaries.filter((summary) => isWeekReview(summary) && !summary.autoGenerated);
}
...
function isWeekReview(summary = {}) {
  return summary.type === 'week' || summary.type === 'weekly' || summary.type === 'weekly-review' || String(summary.id || '').startsWith('week-');
}

function savedReviewRangeLabel(summary = {}) {
  const match = String(summary.range || '').match(/(\d{4}-\d{2}-\d{2})\s*(?:至|-)\s*(\d{4}-\d{2}-\d{2})/);
  return match ? `${formatShortDate(match[1])} - ${formatShortDate(match[2])}` : '';
}
...
function reviewLabel(summary = {}) {
  ...
  const savedRange = savedReviewRangeLabel(summary);
...
function renderReviewPaper(summary) {
  ...
  const start = String(summary.id || '').replace(/^week-/, '');
  const range = isWeekReview(summary) ? savedReviewRangeLabel(summary) || reviewRangeLabel(start) : summary.month;
  return `
